...
import type { CustomerNotificationsPayload } from "@/types/notification";
...
export function AccountNotificationsList() {
  const [payload, setPayload] = useState<CustomerNotificationsPayload | null>(null);
  ...
  const loadNotifications = useCallback(
    async (options?: { silent?: boolean }) => {
      ...
      try {
        const response = await fetch(`/api/account/notifications${queryString ? `?${queryString}` : ""}`, {
          cache: "no-store",
        });
        const data = (await response.json()) as ApiEnvelope<CustomerNotificationsPayload>;
  ...
  return (
    <div className="space-y-4">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Stay updated with order, payment, and account activity.
          </p>
        </div>
        <div className="flex w-full gap-2 sm:w-auto">
          <Button className="flex-1 sm:flex-none" variant="outline" onClick={() => void loadNotifications()}>
      ...
      <section className="grid gap-3 rounded-2xl border border-border bg-card p-4 md:grid-cols-2 xl:grid-cols-4">
        <Input
      ...
      {!payload || payload.items.length === 0 ? (
        ...
        <section className="space-y-3">
          {payload.items.map((item) => {
            ...
            const orderNumber =
              typeof metadata?.orderNumber === "string" || typeof metadata?.orderNumber === "number"
                ? String(metadata.orderNumber)
                : null;
