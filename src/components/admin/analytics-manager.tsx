...
const pieColors = ["#dc2626", "#2563eb", "#16a34a", "#f59e0b", "#7c3aed", "#0ea5e9"];
  ...
  const categoryChartData = useMemo(
    () =>
      (data?.topCategories ?? []).map((item) => ({
        category: shortLabel(item.category, 14),
        revenue: Number(item.revenue.toFixed(2)),
      })),
    [data?.topCategories],
  );
      ...
      {loading ? (
        ...
        <>
          <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            <Card>
              ...
              <CardContent className="space-y-1">
                <p className="text-2xl font-bold">{formatCurrency(data.summary.totalRevenue, "LKR")}</p>
                <p
                  className={`text-xs ${
            ...
            <Card>
              <CardHeader className="pb-2 text-sm text-muted-foreground">Total Orders</CardHeader>
              <CardContent className="space-y-1">
                <p className="text-2xl font-bold">{data.summary.totalOrders}</p>
                <p className="text-xs text-muted-foreground">{data.summary.pendingOrders} pending fulfillment</p>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="pb-2 text-sm text-muted-foreground">Customers</CardHeader>
              <CardContent className="space-y-1">
                <p className="text-2xl font-bold">{data.summary.totalCustomers}</p>
                <p className="text-xs text-muted-foreground">{data.summary.paidOrders} paid orders in range</p>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="pb-2 text-sm text-muted-foreground">Stock Alerts</CardHeader>
              <CardContent className="space-y-1">
          ...
          <section className="grid gap-4 xl:grid-cols-2">
            ...
            <Card>
              <CardHeader className="pb-2 text-sm font-semibold">Top Categories by Revenue</CardHeader>
              <CardContent className="h-[320px]">
                <ResponsiveContainer width="100%" height="100%">
          ...
          <section className="grid gap-4 xl:grid-cols-2">
            <Card>
              <CardHeader className="pb-2 text-sm font-semibold">Customer Growth (6 months)</CardHeader>
                  ...
                  data.lowStockAlerts.map((item) => (
                    ...
                    >
                      ...
                      <p className="mt-1 text-xs text-muted-foreground">
                        Qty: {item.stockQuantity} | Min: {item.minStockLevel} | Status:{" "}
